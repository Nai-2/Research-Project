# -*- coding: utf-8 -*-
"""
🎬🎵 動画×楽曲シンクロ検索 UI (Gradio版)
========================================
起動方法:
    pip install gradio
    python music_sync_ui.py
...
def compute_chroma_sync_score(song_chroma, chunk_infos, step_frames):
    ...
    total = sum(l for _, l, _ in chunk_infos)
    max_start = max(1, song_len - total + 1)
...
def analyze_video(video_path, excluded_genres, progress=gr.Progress()):
    ...
    for ws in window_candidates:
        ...
        if use_chroma:
            ...
            for c, a, i in chunks:
                l = c.shape[1]
                infos.append((np.mean(feat['chroma'][:, i:i + l], axis=1), l, a))
    ...
    for idx, (name, y_sim) in enumerate(top_candidates):
        ...
        if use_chroma and best_win in chroma_by_window:
            ...
            if sc is not None:
                chroma_score = compute_chroma_sync_score(sc, chroma_by_window[best_win], step_frames)
...
def rank_candidates(state, w_mfcc, w_yamnet, w_kw, w_chroma,
    ...
    for i, c in enumerate(ranked):
        rows.append({
            ...
            'KW一致': f"{c['match_count']}件",
            '波形': round(c['mfcc_norm'], 2),
            '雰囲気': round(c['yamnet_norm'], 2),
            'キーワード': round(c['keyword_norm'], 2),
...
PRESETS = {
    ...
    "🌊 波形・展開重視":     (0.70, 0.10, 0.10, 0.10),
    "🎷 雰囲気(YAMNet)重視": (0.10, 0.70, 0.10, 0.10),
    "📝 キーワード意味重視":  (0.20, 0.30, 0.50, 0.00),
    "🎼 メロディ(クロマ)重視": (0.10, 0.15, 0.05, 0.70),
}
